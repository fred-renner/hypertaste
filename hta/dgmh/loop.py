"""The model-orchestrated DGM-H loop (LOOP 1) — grow the AGENT. World-agnostic: it searches
playbook-space for the English disposition that makes a fixed weak student (Haiku) tasteful on a
GIVEN world, and the world is just a `WorldSpec` argument (default: instance 0). This is the
post-mortem's prescription (findings/2026-06-14 §3): "fold into one world-agnostic loop" and run the
real meta + task loop, not a bespoke driver welded to one world.

It reuses the DGM-H machinery wholesale — the archive (`hta.dgmh.archive`), open-ended parent
selection, the Opus self-modify airgap (`hta.dgmh.sandbox`) — and the evolved unit + judge are:

  * evolved node = **`playbook.md` only** — non-executable English, the player's SYSTEM prompt. The
    meta agent (Opus) rewrites THIS, nothing else (safe-eval lifted). A Haiku TOP session natively
    probes / spawns workers / keeps a scratchpad / submits, confined by the probe-MCP airgap.
  * judge = band-normalized **coverage** (`hta.world.grade`): the cells the agent's own probes pin
    and it submits correctly, normalized into the model-free floor->oracle band. Outcome-only,
    agent-inaccessible, deterministic — the integrity floor, lifted to coverage.

"Worlds" are fresh hidden seed draws of the spec (same public structure, a different buried trail
each). Train and a disjoint held-out set are both draws, so held-out coverage measures
generalization across unseen instances. References are spec-constant, so the band is identical for
both splits.

One iteration: seed gen_0000 (the floor playbook) if empty -> select a parent -> draw this
iteration's worlds -> eval the PARENT -> Opus branches + rewrites its playbook into a CHILD -> eval
the CHILD on the SAME worlds -> score (train + held-out) -> archive.
"""
...
from ..world.instances import canonical_spec, draw_hstar
from ..world.language import WorldSpec
from . import sandbox
from .archive import Archive
from .episode.state import EpisodeState, normalize, state_to_env

_HERE = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT = os.path.dirname(os.path.dirname(_HERE))
SEED_DIR = os.path.join(_HERE, "seed")
SERVER_MODULE = "hta.dgmh.episode.server"
...
KICKOFF = ("Investigate this hidden world now. Start by reading world_map, then probe, spawn workers, "
           "and take notes as your instructions direct, and submit_map once when further probing is "
           "not worth its cost.")

# The one-paragraph, HyperAgents-minimal meta prompt. It edits the playbook ONLY, diagnoses from the
# trajectory, and NEVER names "taste" — that is our word for what the lineage discovers, never the
# agent's target (DESIGN.md §4: plant the condition, never the response).
...
# ---------------------------------------------------------------------------
# Worlds: fresh hidden draws of one spec. Parent and child see the SAME draws (a fair comparison);
# each iteration reseeds so a playbook cannot memorize a specific instance.
# ---------------------------------------------------------------------------
def build_worlds(spec: WorldSpec, n_train: int, n_transfer: int, iteration: int):
    base = 10_000 * (iteration + 1)
    train = [(spec, draw_hstar(spec, base + i)) for i in range(n_train)]
    transfer = [(spec, draw_hstar(spec, base + 5_000 + i)) for i in range(n_transfer)]
...
# ---------------------------------------------------------------------------
def run_episode(playbook: str, spec: WorldSpec, hstar, cfg: Config, log=print) -> dict:
    if cfg.backend == "mock":
...
def _mock_floor_player(spec: WorldSpec, hstar) -> dict:
    """The deterministic mock: probe the cheapest coverage-and-probeable cells (the clearings) within
    budget and submit them. Models the no-inference floor — the wiring, not taste."""
...
def _real_episode(playbook: str, spec: WorldSpec, hstar, cfg: Config, log=print,
                  extra_env=None, kickoff: str = KICKOFF, budget=None) -> dict:
    """One live confined episode through the probe-MCP airgap."""
    ...
    try:
        res = llm.episode(prompt=kickoff, model=cfg.task_model,
                          mcp_server_argv=[sys.executable, "-m", SERVER_MODULE],
...
def score_result(spec: WorldSpec, hstar, result: dict):
    """Replay a finished episode's log + submission and score it. Pure: the band judge is a
    deterministic f(structure, observations), so the score does not depend on any live process."""
    st = EpisodeState(spec, hstar)
...
# deterministic. Aggregation is norm-of-mean (average raw, normalize once).
# ---------------------------------------------------------------------------
def evaluate(node_dir: str, worlds: List[tuple], cfg: Config, log=print) -> dict:
    ...
    spec0 = worlds[0][0]
    floor, oracle = grade.floor_value(spec0), grade.oracle_value(spec0)
    per_world = []
...
# ---------------------------------------------------------------------------
def _sanitized_report(per_world: List[dict], spec: WorldSpec, floor, oracle) -> str:
    lines = [
...
# ---------------------------------------------------------------------------
# Meta step: branch the parent node and rewrite its playbook. Real -> Opus through the sandbox airgap
# (Edit/Read/Write, no Bash, never runs the agent against a world). Mock -> a deterministic append so
# the offline plumbing shows a node change.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Run artifacts: the lab's audit log, written next to the archive. The per-iteration record keeps the
# FULL transcripts plus the hidden draws, so any score can be replayed via score_result(spec, hstar,
# result). This file is never on an agent surface, so holding hidden state here leaks nothing.
# ---------------------------------------------------------------------------
...
# One iteration.
# ---------------------------------------------------------------------------
def run_iteration(cfg: Config, iteration: int = 0, spec: Optional[WorldSpec] = None, log=print) -> dict:
