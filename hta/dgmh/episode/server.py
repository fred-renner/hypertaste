"""Stdio MCP server exposing ONE episode as the confined orchestration tools — the airgap. It is a
thin wrapper: every tool delegates to an `EpisodeState` (state.py), which holds the hidden world
(passed in-memory via the server-only env `HTA_WORLD`/`HTA_HSTAR`) and all the load-bearing logic.
The player speaks only JSON-RPC over stdio with no filesystem tools, so it can never read this
process's env (the seed) — it learns the world solely by probing. That tool confinement IS the airgap.
...
Two roles, gated by the toolset the wrapper offers (HTA_ROLE):
  ...
  * **worker** — probe, remaining only. A spawn launches a FRESH worker session (a nested
                 `claude -p`) against an isolated copy of the world with a carved sub-budget; it sees
                 only its task + budget, never the playbook/scratchpad/top context. Depth = 1.

The episode result (log, scratchpad, submission, spawns) is written to `HTA_RESULT_PATH` after every
mutating call — the dropbox the PARENT process reads once the session ends. Scoring lives in the
parent / `EpisodeState.score`, never here.
...
streams + a worker runner so the dispatch, the role airgap, and the spawn carve-out are all exercised
offline without a live `claude -p`.
"""
...
try:
    from .state import EpisodeState, state_from_env
except ImportError:  # pragma: no cover - direct path execution
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))))
    from hta.dgmh.episode.state import EpisodeState, state_from_env
...
_T_PROBE = {"name": "probe",
            "description": ("Reveal the hidden value (a small integer) at one cell, charging its cost "
                            "against the global budget. Only cells flagged `probeable` in world_map "
                            "can be probed; the rest are reconstructed, never probed. "
...
_T_WORLD_MAP = {"name": "world_map",
                "description": ("The public rules of the game: the regions' wiring and the cell layout "
                                "with costs and roles, plus the deterministic value law. The variable "
                                "values are hidden — that is what you probe. probe()/submit_map() use "
                                "the `col` fields here."),
                "inputSchema": {"type": "object", "properties": {}}}
...
_T_SUBMIT = {"name": "submit_map",
             "description": ("End the episode with your reconstruction: an object mapping cell `col` -> "
                             "value for every cell you can determine (coverage cells earn the score). "
                             "Submit once, when further probing is not worth its cost."),
             "inputSchema": {"type": "object",
...
def run_worker(task: str, budget: int, world_env: dict, log_path=None) -> dict:
    """Run a confined worker session and return {observations, used, report}. The worker gets a fresh
    EpisodeState (same world, sub-budget) in its own probe-server process; we read its dropbox after
    it exits. Real backend only."""
    from ... import llm
    ...
    os.close(fd)
    env = dict(world_env)
    env.update({"HTA_BUDGET": str(int(budget)), "HTA_ROLE": "worker", "HTA_RESULT_PATH": result_path})
...
def serve(state: EpisodeState, role: str, instream, outstream,
    ...
    """The MCP framing loop over the given streams. `role` selects the toolset (the airgap); `runner`
    runs a spawn's worker (injected in tests). State mutations are flushed to `result_path` so the
    parent process can read the episode result after the session ends."""
