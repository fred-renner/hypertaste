# VISION (intuition era) — superseded 2026-06-23

> **Status: history, not current.** This was the north star until 2026-06-23. It reached the right
> conclusions but arrived at them from intuition and implementation, and named things by position in
> its own scheme ("the three layers", "layer 2") rather than by function — so it read as jargon a day
> later. The replacement `VISION.md` derives the same conclusions from one starting point (taste is
> the value function of a search with no reward signal) and re-grounds every load-bearing lesson
> below as a *forced consequence* of that start. Nothing here was trashed; what survived was
> re-derived. Kept verbatim as the record of where the framing came from.
