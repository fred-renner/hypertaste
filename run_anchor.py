...
is hard (B2):
  ...
  2. SWEEP the gap dial (the valley-to-clearing mass ratio `Lv`, the budget, the probe cost) and
...
The gap lives in a **trail-gated payoff under deception**. A *trail* of signpost registers
(trailhead -> waypoint -> landmark) gates a big valley; reading a signpost pays ZERO coverage (it is an
instrument, not a map cell) and the valley is inference-only (reconstructed, never drilled). So no
prefix of the trail pays — a bounded planner cannot climb it — and because there are at least as
many fat *clearing* blocks as the budget, spending a probe on a zero-coverage signpost has a strict
opportunity cost. The greedy/2-step planner takes the clearings; only the full oracle commits the
three probes to the landmark. Reconstruction stays a lookup throughout (B2), so the gap survives an
English playbook (you cannot brute-force a trail search in prose) and the landmark's identity is
...
def candidates():
    ...
    cost knob, and the canonical anchor. `waypoints`/`landmarks` are the public pointer tree; the hidden
    seed (register values) fixes where the realized trail ends."""
    base = dict(R=10, K=2, Ld=2, Lv=9, trailhead=0, waypoints=(1, 2), landmarks=((3, 4), (5, 6)), budget=3)
    return [
        # --- controls: no deep payoff / slack budget -> allocation is trivial -> below ---
        TrailSpec("ctrl-novalley", **{**base, "Lv": 0}),
        TrailSpec("ctrl-slack",   **{**base, "budget": 6}),      # clearings < budget -> trail climbable
        # --- the gap dial: the valley-to-clearing mass ratio Lv (bigger valley -> bigger gap, steeper ramp) ---
        ...
        # --- the variable-cost virtue: a costlier clearing sharpens the cost-weighted gap ---
        TrailSpec("trail-cost",   **{**base, "cost_clearing": 2, "budget": 4}),
    ]
...
def main():
    ...
    print("=" * 104)
    base = dict(R=10, K=2, Ld=2, Lv=9, trailhead=0, waypoints=(1, 2), landmarks=((3, 4), (5, 6)), budget=3)
    ctrl = screen(TrailSpec("ctrl-novalley", **{**base, "Lv": 0}))
    ...
    if not band:
        print("  No spec clears all three — widen the valley-to-clearing ratio or tighten the budget.")
        return
