...
basket, and the separation of a below-threshold control from the chain-gated trail.
...
- Reconstruction is a LOOKUP, not a solve (B2): pinning the three trail registers determines the
  whole vault; the vault is never probed.
...
# A small, fast trail (256 hypotheses): root -> relay(1|2) -> lode(3|4), three lure blocks, vault
...
TRAIL = TrailSpec("trail-small", R=8, K=2, Ld=2, Lv=9, root=0,
                  relays=(1, 2), lodes=((3, 4), (3, 4)), budget=3)
CONTROL = TrailSpec("ctrl-novault", R=8, K=2, Ld=2, Lv=0, root=0,
                    relays=(1, 2), lodes=((3, 4), (3, 4)), budget=3)
...
def test_expander_deterministic_and_bounded():
    ...
    # the vault is inference-only: every vault column is scored as coverage but never probeable
    vault_cols = [i for i, c in enumerate(cells) if c[0] == "vault"]
    assert vault_cols and all(i in cov and i not in probe for i in vault_cols)
...
def test_reconstruction_is_a_lookup_not_a_solve():
    # B2: once the three trail registers (root, relay, lode) are pinned, the whole vault is
    # determined by consensus — no joint solve, the deep payoff is a lookup. Pin them for one true
    # world and check every vault cell is determined.
    table, cells, costs, cov, probe = build_tableau(TRAIL)
    import itertools
    hyps = list(itertools.product(range(TRAIL.K), repeat=TRAIL.R))
    h = hyps[0]
    chain = TRAIL.chain_regs(h)
    assert len(chain) == 3                               # depth-3 trail (root, relay, lode)
    H = frozenset(i for i, hh in enumerate(hyps) if all(hh[r] == h[r] for r in chain))
    vault_cols = tuple(i for i, c in enumerate(cells) if c[0] == "vault")
    assert determined(table, vault_cols, H) == TRAIL.Lv  # all Lv vault cells pinned by the chain
    # ...and pinning only two of the three leaves the vault unresolved (non-submodular, deep)
    H2 = frozenset(i for i, hh in enumerate(hyps) if all(hh[r] == h[r] for r in chain[:2]))
    assert determined(table, vault_cols, H2) == 0
...
def test_control_is_below_threshold():
    # No deep payoff -> coverage is just the lure blocks -> greedy is optimal -> the best
    ...
    assert s["ramp_r2"] > 0.999  # R^2 == 1.0: pure linear lure mass, the independent-world signature


def test_trail_is_above_threshold():
    # The chain-gated payoff under deception makes the optimal allocation non-shallow: the oracle
...
def test_ramp_is_monotone_and_anti_cliff():
    ...
    assert max(steps) <= 0.55                   # anti-cliff (the lure mass flattens the vault step)
...
def test_determined_is_agent_inaccessible_and_exact():
    ...
    n_cov = len([c for c in cells if c[0] in ("direct", "vault")])
    assert determined(table, cov, frozenset({0})) == n_cov          # one world -> all coverage pinned


def test_variable_cost_sharpens_the_gap():
    # The budget is a COST budget: a costlier lure raises the opportunity cost of the trail's
    # zero-coverage signposts, widening (never inverting) the gap. The cost knob is real.
    cheap = screen(TrailSpec("cheap", R=8, K=2, Ld=2, Lv=9, root=0,
                             relays=(1, 2), lodes=((3, 4), (3, 4)), budget=4))
    dear = screen(TrailSpec("dear", R=8, K=2, Ld=2, Lv=9, root=0,
                            relays=(1, 2), lodes=((3, 4), (3, 4)), budget=4, cost_lure=2))
